/**
 * This code snippet demonstrates the use of the C++ Standard Library's vector class, which is a dynamic array 
 * that can resize itself automatically when elements are added or removed. The program initializes two vectors of integers, 
 * displays their sizes and contents, allows the user to input values into the vectors, and then performs various operations 
 * such as comparing the vectors, copying one vector to another, and accessing elements using both subscript and at() methods. 
 * It also includes error handling for out-of-range access and demonstrates how to change the size of a vector using push_back().
 * 
 ...
 * 
 *      g++ -Wall -Werror -std=c++11 vec_book_exmp.cpp -o test
 * 
 */
